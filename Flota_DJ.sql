-- Creamos el esquema nuevo
-- New schema creation

CREATE SCHEMA flota_dj ;

-- Creamos las tablas
-- Tables creation

CREATE TABLE flota_dj.grupo(
	id serial primary key,
	nombre varchar(50) not null
);

CREATE TABLE flota_dj.marca(
	id serial primary key,
	nombre varchar(50) not null
);

CREATE TABLE flota_dj.modelo(
	id serial primary key,
	nombre varchar(50) not null
);

CREATE TABLE flota_dj.color(
	id serial primary key,
	nombre varchar(50) not null
);

CREATE TABLE flota_dj.aseguradora(
	id serial primary key,
	nombre varchar(60) not null
);

CREATE TABLE flota_dj.moneda(
	id serial primary key,
	nombre varchar(60) not null
);

CREATE TABLE flota_dj.mantenimiento(
	id serial primary key,
	...
	constraint pk_moneda foreign key(id_moneda) references flota_dj.moneda(id)	
);

CREATE TABLE flota_dj.vehiculo(
	matricula varchar(15) primary key,
	id_grupo int not null,
	constraint pk_grupo foreign key(id_grupo) references flota_dj.grupo(id),
	id_marca int not null,
	constraint pk_marca foreign key(id_marca) references flota_dj.marca(id),
	id_modelo int not null,
	constraint pk_modelo foreign key(id_modelo) references flota_dj.modelo(id),
	id_color int not null,
	constraint pk_color foreign key(id_color) references flota_dj.color(id),
	fecha_compra date not null,
	kms_totales int not null
);

alter table flota_dj.mantenimiento add constraint pk_vehiculo
foreign key(matricula) references flota_dj.vehiculo(matricula);

CREATE TABLE flota_dj.seguro(
	id serial primary key,
	matricula varchar(15) not null,
	constraint pk_matr foreign key(matricula) references flota_dj.vehiculo(matricula),
	id_aseguradora int not null,
	constraint pk_asegur foreign key(id_aseguradora) references flota_dj.aseguradora(id),
	poliza varchar(30) not null,
	fecha_alta date
);

-- Importamos los datos a una tabla nueva
-- Data importation into a new table

CREATE TABLE flota_dj.datos (
...
INSERT INTO flota_dj.datos (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,poliza,fecha_alta,importe,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
insert into flota_dj.grupo (nombre) select grupo from flota_dj.datos group by grupo;
insert into flota_dj.marca (nombre) select marca from flota_dj.datos group by marca;
insert into flota_dj.modelo (nombre) select modelo from flota_dj.datos group by modelo;
insert into flota_dj.color (nombre) select color from flota_dj.datos group by color;
insert into flota_dj.aseguradora (nombre) select aseguradora from flota_dj.datos group by aseguradora;
insert into flota_dj.moneda (nombre) select moneda from flota_dj.datos group by moneda;

insert into flota_dj.vehiculo (matricula, id_grupo, id_marca, id_modelo, id_color, fecha_compra, kms_totales)
select da.matricula, gr.id, ma.id, mo.id, co.id, da.fecha_compra, da.kms_totales
	from flota_dj.datos da
	join flota_dj.grupo gr
	on gr.nombre = da.grupo
	join flota_dj.marca ma
	on ma.nombre = da.marca
	join flota_dj.modelo mo
	on mo.nombre = da.modelo
	join flota_dj.color co
	on co.nombre = da.color
	group by da.matricula, gr.id, ma.id, mo.id, co.id, da.fecha_compra, da.kms_totales;

insert into flota_dj.seguro (matricula, id_aseguradora, poliza, fecha_alta)
select da.matricula, ase.id, da.poliza, da.fecha_alta
	from flota_dj.datos da
	join flota_dj.aseguradora ase
	on da.aseguradora = ase.nombre;

insert into flota_dj.mantenimiento (matricula, fecha_revision, kms_revision, importe, id_moneda)
select da.matricula, da.fecha_revision, da.kms_revision, da.importe, mon.id
	from flota_dj.datos da
	join flota_dj.moneda mon
	on da.moneda = mon.nombre;

-- Query de prueba
-- Test Query

select v.matricula, gr.nombre, ma.nombre, mo.nombre, co.nombre, v.kms_totales, v.fecha_compra, ase.nombre, seg.poliza, seg.fecha_alta
from flota_dj.vehiculo v
join flota_dj.grupo gr
on v.id_grupo = gr.id
join flota_dj.marca ma
on v.id_marca = ma.id
...
join flota_dj.color co
on v.id_color = co.id
join flota_dj.seguro seg
on v.matricula = seg.matricula
join flota_dj.aseguradora ase
on ase.id = seg.id_aseguradora
group by v.matricula, gr.nombre, ma.nombre, mo.nombre, co.nombre, v.kms_totales, v.fecha_compra, ase.nombre, seg.poliza, seg.fecha_alta
